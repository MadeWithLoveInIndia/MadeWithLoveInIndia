<?php
	...
	$user = DB::queryFirstRow("SELECT id FROM users WHERE passwordresetcode=%s", $recoverCode)["id"];
	if (!$user) {
		...
		if (isset($_POST["password"])) {
			...
			if ($result === FALSE || json_decode($result)->success == false) {
				...
				header("Location: /login?message=Your+password+was+successfully+changed.+You+may+log+in+now.");
			}
		}
	}
